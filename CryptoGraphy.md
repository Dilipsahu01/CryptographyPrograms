# **1 — Digital Design & VLSI Fundamentals**
...
Confidentiality is solved by Encryption (e.g., AES, ChaCha20).  
Integrity is solved by Hashing (e.g., SHA-256).  
Authentication is solved by Digital Signatures (e.g., RSA Signatures).  
Non-Repudiation is solved by Digital Signatures.  
Availability is solved by hardware and system Redundancy & Recovery.

## **3 — Core Cryptographic Algorithms**
...
**Key Generation:**  
   ...
   5\. Compute the private key d (the modular inverse) such that d × e ≡ 1 mod φ(n).  
Usage: The public key is (e, n) and the private key is (d, n).
...
**The Perfect Combo**: No single algorithm does everything. A modern secure connection (like loading a webpage) uses Asymmetric Cryptography (to securely trade a temporary secret key), Symmetric Cryptography (to rapidly encrypt the heavy video/web traffic using that temporary key), and Digital Signatures (to prove the server is actually Google and not an imposter.).
...
**The Problem:** If you encrypt your entire database and then accidentally lose the only copy of the key, your business is dead. Data is gone forever.

**Threshold Cryptography & Shamir's Secret Sharing:** A mathematical way to split a master key into n different pieces (e.g., 5 pieces given to 5 executives). You require a minimum threshold of k pieces (e.g., any 3 of the 5\) to mathematically reconstruct the master key. This prevents a single rogue employee from stealing the key, while ensuring the key isn't lost if one executive loses their piece.

**Key Backup**: Storing heavily encrypted backups of master keys in physically secure, offline vaults (like cold storage).

### **4.5 — Random Number Generation**

**Importance:** All cryptography is entirely dependent on randomness. If an attacker can guess how your computer generates "random" numbers, they can predict your AES and RSA keys without doing any heavy math.

**PRNG (Pseudo-Random Number Generator):** Algorithms that use math to spit out numbers that look random, but are actually deterministic. They are incredibly fast but require a truly random "seed" to start.

**TRNG (True Random Number Generator):** Hardware circuits that harvest actual physical chaos from the universe (entropy) to generate numbers.

**Hardware Entropy Sources:** Engineers build circuits that measure unpredictable physical phenomena like thermal noise, electrical jitter, or ring oscillator drift.

**Standards:** NIST DRBG (Deterministic Random Bit Generator) outlines exactly how to build secure RNGs combining physical TRNGs with fast PRNGs.

## **5 — Post-Quantum Cryptography (PQC)**

### **5.1 — Why PQC?**

**The Quantum Threat:** Standard computers use bits (0 or 1). Quantum computers use qubits, which can represent complex states and solve specific math problems exponentially faster than supercomputers.

**Shor's Algorithm:** A quantum algorithm that easily finds the prime factors of massive numbers. If a sufficiently large quantum computer is built, it will instantly break RSA and Elliptic Curve Cryptography (ECC), effectively destroying modern internet security.

**The Goal of PQC**: To invent new cryptographic algorithms running on classical hardware (our current CPUs and FPGAs) that involve math so complex that even a future quantum computer cannot crack it.
...
**The Hardware Bottleneck:** Lattice cryptography requires multiplying massive polynomials together. Doing this the standard way is incredibly slow and blocks real-time encryption.

**FFT Intuition:** Similar to how the Fast Fourier Transform (FFT) speeds up audio signal processing, the NTT speeds up polynomial multiplication over a finite field.

**The NTT Flow**: Instead of multiplying directly, you transform the polynomials using NTT, perform simple piece-by-piece multiplication, and then run an Inverse-NTT to get the final answer. This is the absolute most critical logic block to design in a PQC hardware accelerator.

### **5.4 — NIST Standardized Algorithms**

**FIPS 203 (ML-KEM):** The official standard for Key Encapsulation (securely trading keys over the internet). It is Lattice-based and replaces Diffie-Hellman/ECDH.

 **Hardware Flow:** \`Input Polynomial → NTT Engine → Polynomial Multiply → Inverse NTT → Compression → Ciphertext\`

**FIPS 204 (ML-DSA):** The official standard for Digital Signatures. It is Lattice-based and replaces RSA Signatures and ECDSA.

**Hardware Flow:** \`Message → Hash → Polynomial Sampling → NTT → Matrix Operations → Signature\`

**FIPS 205 (SLH-DSA):** A backup signature standard based purely on Hashing math. It has massive signatures but relies on very proven math in case Lattices are ever broken.

### **5.5 — Additional Standards**

**FIPS 206 (FN-DSA / Falcon):** Another Lattice-based signature algorithm. It is harder to implement in hardware but results in significantly smaller signatures than ML-DSA. Expected to be officially standardized in 2026\.

**HQC (Hamming Quasi-Cyclic):** A Code-based Key Encapsulation mechanism. It serves as NIST's official backup plan in case a mathematical flaw is discovered in the Lattice-based ML-KEM.

### **5.6 — Signature Competition Candidates (NIST Round 3\)**

**The Ongoing Search:** While ML-DSA and SLH-DSA are standardized, they are computationally heavy and have large key sizes compared to legacy ECC. NIST is currently running an ongoing competition to find smaller, faster digital signatures not based on lattice math.

**Top Candidates:** FAEST, MQOM, SDitH, MAYO, and ONOVA. Hardware engineers must watch these closely, as they will dictate the next generation of IoT security chips.

### **5.7 — Quantum Resistant Symmetric Security**

### **6.8 — FPGA Design Flow**

**Symmetric Safety:** While quantum computers destroy asymmetric math (RSA/ECC) using Shor's Algorithm, they only halve the security of symmetric math (AES/SHA) using Grover's Algorithm.

**The Fix:** You do not need entirely new algorithms for symmetric crypto. Simply doubling the key size is enough.

**Standard:** AES-256 and SHA-384/SHA-512 are universally considered completely secure against all known future quantum attacks.

## **6 — FPGA Architecture & Verilog Implementation**

Check out the [Source Code](https://github.com/Dilipsahu01/CryptographyPrograms) for these implementations.
...
### **6.2 — Implementation Architectures**

**Iterative Design:** Reuses the exact same hardware logic block for every round of cryptography (e.g., passing data through one AES round hardware block 10 times). Saves massive chip area, but has high latency.

**Fully Unrolled Design:** Builds 10 separate, back-to-back AES round blocks. Massively increases chip area, but drastically reduces latency (data shoots straight through in one shot).

**Pipelined Design:** Adding registers between an unrolled design. Data flows like an assembly line, allowing you to encrypt a brand new plaintext block on every single clock cycle (highest possible throughput).

### **6.3 — Verilog Programming**

**What is Verilog?:** A Hardware Description Language (HDL). You are not writing software; you are literally wiring up physical logic gates using text.
...
**The Golden Rule:** In Verilog, all \`assign\` statements execute simultaneously in real-time. Code line order does not matter. Inside an \`always\` block, order matters sequentially.
...
**Metastability & CDC:** When data crosses from a slow clock domain (e.g., a 50MHz CPU bus) to a fast clock domain (e.g., a 200MHz Crypto Core), the signal might arrive exactly as the fast clock ticks. The flip-flop gets confused and outputs a voltage halfway between 0 and 1\. We fix this dangerous issue using FIFOs and multi-stage Synchronizers.
...
**Resource Metrics:** How much of the FPGA chip did you use?   
about LUT Utilization, Flip-Flop (FF) Utilization, DSP count, BRAM usage, and total Power Consumption.

**The Tradeoff:** Unrolling loops and parallelizing math dramatically increases speed (Throughput), but instantly balloons the physical size (Area) and battery drain (Power).

### **6.7 — Verification & Validation**

**The Cost of Bugs:** Software bugs can be patched over the air. Hardware bugs baked into an ASIC cost millions of dollars and months to fix.  
...
### Pipeline: Designing a complete hardware system involves understanding the math (Algorithm) → wiring it in Verilog (RTL Design) → proving it works (Simulation) → converting to gates (Synthesis) → meeting physical physics constraints (Timing Closure) → testing it on a real board (Deployment) → measuring Gigabits/second (Performance Measurement).

## **7 — System-on-Chip (SoC) Integration & Bus Interfaces**

### **Context**

**Bridging the Gap:** Building an incredibly fast AES Verilog core (Topic 6\) is completely useless if the CPU cannot talk to it. SoC integration is the engineering art of connecting your custom hardware accelerator directly to the main processor so software applications can actually use it.
...
**The Information Highway:** AXI (Advanced eXtensible Interface) is an industry-standard communication bus created by ARM. It dictates exactly how the CPU and hardware accelerators pass data back and forth.

**AXI4 (Full)**: High-performance, high-bandwidth memory-mapped bus. Used for moving massive chunks of data (like a 4GB movie) into the crypto core via bursts.

**AXI Lite**: A lightweight, simple bus. Used purely for the CPU to configure the crypto core (e.g., flipping a single bit to say "Start Encryption" or writing the 256-bit AES key to a specific register).

**AXI Stream:** A continuous, high-speed firehose of data flowing strictly in one direction. It has no memory addresses—just a raw stream of plaintext streaming in, and ciphertext streaming out.

### **7.3 — APB Protocol**

**Advanced Peripheral Bus (APB):** A much slower, simpler, and lower-power bus compared to AXI. It is generally used to connect low-speed peripherals (like timers, UARTs, or simple RNGs) to the CPU where blistering speed isn't required.

### **Memory Design and Organization**

**Memory Hierarchy:** A tiered structure (L1/L2/L3 cache, RAM, storage) that balances speed, cost, and capacity to keep the CPU fed with data.

**Memory Classification:** Categorizes memory by volatility (retains data without power) and access patterns (random access vs. sequential streams like tape).

**Semiconductor Memory:** Distinguishes between SRAM (fast, stable, 6-transistor cells) and DRAM (high density, 1-transistor/1-capacitor, requires refreshing).

**SDRAM Evolution:** The progression of synchronized memory standards from basic SDR to the high-bandwidth, power-efficient DDR5 era.

**Processor-Memory Interface:** The hardware paths and logic (decoding/interleaving) that connect CPU addresses to physical memory slots for efficient access.

**Cache Memory & Mapping:** Techniques like direct, fully associative, and set-associative mapping that manage data location using valid and dirty bits to track updates.

**Cache Replacement Policies:** Algorithms such as LRU (Least Recently Used), FIFO, and Random that determine which data to discard when the cache is full.

**Locality of Reference:** Utilizes the principle that programs access a small cluster of data repeatedly (temporal) or in sequence (spatial) to optimize cache hits.

**Secondary Storage:** Long-term data storage using magnetic platters (HDDs) or flash-based NAND (SSDs), measured by access and transfer latency.

### **Controllers and Input/Output (I/O) Organization**

**I/O Module Interfaces:** The circuitry used to translate processor signals into formats compatible with diverse peripheral hardware.

**Device Mapping:** Approaches where I/O devices occupy either specific memory addresses (Memory-Mapped) or their own isolated port space (I/O-Mapped).
...
**DMA Calculations:** Metrics used to quantify the impact of DMA on system performance, including transfer rates and CPU overhead.

### **Measuring CPU Performance**

**Execution Metrics:** Formulas using clock cycles, frequency, and Cycles Per Instruction (CPI) to determine the absolute time a program takes to execute.

**Amdahl's Law:** A mathematical model used to predict the maximum theoretical speedup of a system when only a portion of the workload is improved.

**Architecture Tradeoffs:** The design balance between instruction set types—RISC (fewer, simple instructions) versus CISC (complex, multi-cycle instructions) and their impact on CPI.

### **Pipelining and Advanced Architectures**

**Arithmetic Pipelining:** Breaking down complex mathematical operations (like addition) into smaller stages to allow continuous, overlapping calculation.

**MIPS32 Instruction Cycle:** A classic 5-stage architectural model (Fetch, Decode, Execute, Memory, Write-back) that optimizes instruction throughput and interrupt handling.

**Vector Processors:** Advanced architectures that use single instructions to operate on entire arrays (vectors) of data simultaneously, maximizing computational efficiency.

### **7.5 — Crypto Accelerator Integration**
...
## **8 — Hardware Security & Countermeasures**

### **8.1 — Side Channel Attacks (SCA)**

**The Reality:** Hackers don't try to break the AES math (which is nearly impossible). Instead, they attack the physical chip doing the math.

**Leakage Sources**: As the chip calculates the key, it leaks tiny amounts of physical information: spikes in Power Consumption, fractional millisecond Timing differences, or Electromagnetic (EM) Radiation.

**The Attack**: By statistically analyzing these tiny power drops while the chip encrypts data, the attacker can literally extract the AES key bit by bit.

**Countermeasures:** Hardware engineers must design "Constant-Time Execution" (where math always takes the exact same amount of time) and build Hardware Noise Generators to mask the real power spikes in static noise.

### **8.2 — Fault Injection (FI) Attacks**

**The Attack:** An attacker physically drops the voltage for a microsecond (Voltage Glitching), blasts a laser at the silicon (Laser Injection), or spikes the clock signal (Clock Glitching).

**The Goal:** This physical shock causes the CPU to skip a single instruction. If the skipped instruction was \`if (password\_is\_correct)\`, the system unlocks itself.

**Countermeasures:** Designing Redundant Computation (doing the same math twice and comparing the results). If a laser causes the first calculation to glitch, the results won't match, and the chip instantly self-destructs the operation.
...
### **8.4 — Hardware-Specific Defenses**

**Device Binding:** Encrypting data in such a way that it is mathematically bound to the physical CPU's serial number or MAC address. If the hard drive is moved to another computer, it refuses to decrypt.

**Hardware Root of Trust:** A physically isolated, highly secure enclave inside the SoC (like Apple's Secure Enclave) that holds the master keys and never lets them touch the main OS.

### **8.5 — Crypto Agility**

**The Problem:** If you hardcode a cryptographic algorithm into permanent ASIC silicon, and that algorithm is broken 5 years later, the chip becomes e-waste.

**The Solution:** Designing modular software libraries paired with reconfigurable FPGA hardware accelerators so the physical device can simply download an update to support new Post-Quantum math when the landscape changes.

### **8.6 — Secure Boot**

**The Boot Chain:** When power turns on, the system runs from an unalterable, read-only memory (ROM). The ROM uses Digital Signatures to verify that the Bootloader hasn't been hacked. The Bootloader verifies the Operating System.

**The Result:** A chain of cryptographic trust ensuring that the device is running exactly the firmware the manufacturer intended, blocking rootkits before the OS even starts.

## **Final Outcome: The Learning Progression**

The knowledge flows in this logical sequence:

Digital Logic → Computer Architecture & I/O → Cryptography & Security → Post-Quantum Cryptography → FPGA Design → SoC Integration → Hardware Security

This progression models the foundations (Boolean logic and computer architecture), move to specialized domains (cryptographic algorithms and post-quantum security), and finally learn to implement and defend these systems (FPGA design, SoC bus integration, and physical security countermeasures).